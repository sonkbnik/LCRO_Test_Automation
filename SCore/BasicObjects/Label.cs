...
{
    public class Label : IClick, IGetText, IVisible
    {
        ...
        public string Text => DriverActions.GetText(Locator, Name);
        ...
        {
            Console.WriteLine("LABEL: " + (WebElement)ExpectedCondition.WaitUntilElementExists(Locator));
            return (WebElement)ExpectedCondition.WaitUntilElementExists(Locator);
        }
        public WebElement IsElementClickable()
        {
            WebElement ele = (WebElement)ExpectedCondition.IsElementClickable(Locator);
            return ele;
        }
    }
}
